...
def get_logger():
    ...
    file_handler = logging.FileHandler(base_path + log_update_path + '/steam_game_updater.log')
...
def delete_duplicates():
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
            logger.info(f"Read {len(data)} entries from the file before removing duplicates.")

        if not data:
            logger.warning("The file is empty. Skipping duplicate removal.")
            return
...
def update_game_list():
    ...
    try:
        ...
        if os.path.exists(base_file_path):
            ...
            logger.warning("No existing game database found, creating a new one.")
            base_data = []
        ...
        if not missing_games:
            ...
            logger.info("No missing games found. Database is up to date.")
        ...
        if os.path.exists(base_file_path):
            ...
            logger.info(f"Removed old base file: {base_file_path}")
        os.rename(new_file_path, base_file_path)
        logger.info(f"Updated base file: {base_file_path}")
        ...
        logger.info("Updated timestamp of the last database update with new games information.")
